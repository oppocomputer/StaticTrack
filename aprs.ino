...
void CheckAPRS(void) //Function to decide when to transmit a message.
{
  if ((millis() >= NextAPRS) && (GPS.Satellites >= minimumSatellites) && (_txlen == 0))
  {   
...
void tx_aprs(void)
{
  ...
  char stlm[9];
  char *ptr;
  static uint16_t messagesTransmitted = 0;
  ...
  aprs_alt = GPS.Altitude;

  float latitude = float(GPS.Latitude);
  float longitude = float(GPS.Longitude);
  int altitude = GPS.Altitude;
  
  char buffer_latitude[9];
  char buffer_longitude[9];
  
  dtostrf(latitude,7,5,buffer_latitude); //place latitude into buffer
  dtostrf(longitude,7,5,buffer_longitude); //place longitude into buffer
...
if (aprs_mode == 0)
  { 
    char data[200];
    sprintf(data,"%s,%s; %d m; www.dirkgeeroms.be",buffer_latitude,buffer_longitude,aprs_alt); //fill data with information
    
    //Serial.print(latitude,longitude); //testing
    ...
    ax25_frame(
      ...
      APRS_DEVID, 0,
      Wide1Path, Wide2Path,
      data //data is the variable containing our message
    );

      // Original message
      //ax25_frame(
      //  "Lat=%E | Lon=%E  H=%ld m| %s",
      //  latitude,
      //  longitude,
      //  ax25_base91enc(slat, 4, aprs_lat),
      //  ax25_base91enc(slng, 4, aprs_lon),
      //  aprs_alt, APRS_COMMENT);  // comment,APRS_CALLSIGN, ++APRSSentenceCounter);

    
    #ifdef APRS_TELEM_INTERVAL
      // Send the telemetry definitions every n packets (n = telemetry interval)
      if( messagesTransmitted % (APRS_TELEM_INTERVAL) == 0){
        aprs_mode = 0;
      }
      else {
        aprs_mode = 1;
...
if (aprs_mode == 0)
  {
    //EDIT THIS!!! to change output packet
    ax25_frame(
      APRS_CALLSIGN, APRS_SSID,
      APRS_DEVID, 0,
      Wide1Path, Wide2Path,
      ...
      //ax25_base91enc(slat, 4, aprs_lat),
      //ax25_base91enc(slng, 4, aprs_lon),
      //altitude, APRS_COMMENT, APRSSentenceCounter);  // comment,APRS_CALLSIGN, ++APRSSentenceCounter);
      ++APRSSentenceCounter;
    #ifdef APRS_TELEM_INTERVAL
...
#ifdef APRS_TELEM_INTERVAL  
#define APRS_PARM1    "PARM1"
#define APRS_UNIT1    "UNIT1"
#define APRS_EQNS1    "EQNS1"

 #ifdef WIREBUS
  #define APRS_PARM2   "PARM2"
  #define APRS_UNIT2   "UNIT2"
  #define APRS_EQNS2   "EQNS2"
 #endif  
  
  #define APRS_PARM3   "PARM3"
  #define APRS_UNIT3   "UNIT3"
  #define APRS_EQNS3   "EQNS3"
  
#define APRS_EQNS4     "EQNS4"
